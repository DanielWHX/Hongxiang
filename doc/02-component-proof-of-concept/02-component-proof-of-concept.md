...
Chosen -> TCG Marketplace
   1. Core Business Relevance
      this component intergrate TikTok Shop with an independent e-commerce site
      this component's functionality is to emabling transactions, price tracking, and dynamic inventory updates
   2. Key Functionalities for MVP
      Listing items for sale
      Buying items
      Tracking item availability and price changes
   3. Expandability
      future I want the price trends with real time market price, referenced by TCGplayer
      Integrate with TikTok Shop API
      Integrate with AI model deepseek for data analysis
   4. Feasibility
      Market Price: I will use MongoDB to store history price and priceHistory component to record price changing which is referenced by TCG player(API)
      Transaction security: use OAuth / JWT and paypal
      Performance:
         Node.js + Express Suitable for high concurrent requests.
         MongoDB can track price changing
         Redis It can cache the prices of popular products to reduce the pressure of database queries.
